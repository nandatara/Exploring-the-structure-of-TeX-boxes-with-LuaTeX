--[[
Patrick Gundlach has granted Overleaf permission to use and distribute this modified version of his original code---which
can be found at http://gist.github.com/556247
This modified version has been renamed "hiviznodelist.lua"
]]
-- viznodelist.lua
-- speedata publisher
...
module("hiviznodelist",package.seeall)
...
if n then
	...
	if t=="temp" or t=="nested_list" then 
    	return 
     end
...
local ret = {}
	...
	local nodename = get_nodename(n)
-- I don't want to list the attributes: commented 
--[[	if n.id ~= 50 then
...
if typ == "hlist" then
		...
		--if head.glue_set ~= 0 then
			local glue_set = string.format("glue_set %f",head.glue_set)
		...
		tmp[#tmp + 1] = {"list", "list (of nodes in this box)"}
...
elseif typ == "glue" then
	...
	local spec = string.format("%gpt", head.width / 2^16)
	--if head.stretch ~= 0 then
		local stretch_order, shrink_order
			if head.stretch_order == 0 then
				stretch_order = string.format(" + %gpt",head.stretch / 2^16)
			else
				stretch_order = string.format(" + %g fi%s", head.stretch / 2^16, 							string.rep("l",head.stretch_order - 1))
			end
 	spec = spec .. stretch_order
	--end

	--if head.shrink ~= 0 then
		if head.shrink_order == 0 then
			shrink_order = string.format(" - %gpt",head.shrink / 2^16)
		else
			shrink_order = string.format(" - %g fi%s", head.shrink / 2^16, 								string.rep("l",head.shrink_order - 1))
		end
...
elseif typ == "rule" then
	...
	local wd,ht,dp
	if head.width == -1073741824 then 
       	wd = "width: flexible" 
    else 
       	wd = string.format("width: %gpt", head.width / 2^16)
    end
        
	if head.height == -1073741824 then 
		ht = "height: flexible" 
	else 
		ht = string.format("height: %gpt", head.height / 2^16) 
	end
    
	if head.depth == -1073741824 then 
    	dp = "depth: flexible" 
    else 
    	dp = string.format("depth: %gpt", head.depth / 2^16) 
    end
